...
- **Neutrals:** `text` for anything read first. `text-2` for secondary text read in full: sidebar links, idle chips and page numbers, review excerpt and author, auth links, `.tag`, the wordmark tails. `muted` for meta: dates, counts, sort links, group heads, track numbers and durations, artist line, placeholders, IP tag. `muted-2` for decoration only: the reply elbow, the sort hover underline, the scrollbar thumb. `line` is the `.tag` ring and nothing else.
...
- **Display:** `brand` 22px/1.2/900 for the wordmark (capitals D·U·L·I on an even 1.5em pitch, `.KR` in the same sweep, 10px tails), `album-title` 30px/1.25/900 with `word-break: keep-all`, `score` 28px/1.2/900 in `accent-ink`, tabular.
